...
try:
    ...
    pass
#sys.stdout = open("results_unoptimised.txt", "w")
####################################### Utility functions #######################################################
...
for p1 in reg_users:
    ...
    for i in range(mxlen):
        data['user'].append(p1)
        data['class'].append(0 if p1[0]=="I" else 1)
...
for f in folds:
    ...
    for i in list(frame.index.values):
        d[frame['class'][i]][frame['cluster'][i]]+=1
    ...
    if 0 in d[0]:
        ...
        d[1]=1-d[1].index(0)
        d[0]=1-d[1] 
    elif sum(d[0])>sum(d[1]):
        d[0]=d[0].index(max(d[0]))
        d[1]=1-d[0]
    else:
        d[1]=d[1].index(max(d[1]))
        d[0]=1-d[1]
    ...
    total=0
    for i in list(frame.index.values):
        if frame['cluster'][i]==d[frame['class'][i]]:
            score+=1
        total+=1
    acc=score/total
    acc*=100
